...
class SaleImportExportService:
    ...
    # Add these new methods to your SaleImportExportService class

    @staticmethod
    def start_incremental_import(import_id, user_id):
        """
        Start an incremental import process

        Returns a session object that can be used for monitoring progress
        """
        try:
            # Find the temporary import record
            temp_import = TempImport.get_by_id(import_id, user_id)
            ...
            # Initialize progress tracking for each record
            temp_import.progress = {
                "total": len(valid_records),
                "processed": 0,
                "succeeded": 0,
                "failed": 0,
                "record_statuses": {str(i): {"status": "pending", "error": None, "timestamp": datetime.now().isoformat()} for i in range(len(valid_records))}
            }
    ...
    @staticmethod
    def import_next_record(import_id, user_id):
        """
        Import the next pending record in the import session

        Returns the status of the operation and updated progress
        """
        try:
            # Find the temporary import record
            temp_import = TempImport.get_by_id(import_id, user_id)
            ...
            if temp_import.status not in ["in_progress", "pending"]:
                return {
                    'success': False,
                    'error': f'Import is already {temp_import.status}'
                }
            ...
            # Find the next pending record
            next_index = None
            for i in range(len(valid_records)):
                status = temp_import.progress["record_statuses"].get(
                    str(i), {}).get("status")
            ...
            if next_index is None:
                ...
                if temp_import.progress["failed"] == 0:
                    temp_import.status = "completed"
                else:
                    temp_import.status = "partially_completed"
            ...
            record = valid_records[next_index]
            ...
            try:
                ...
                if error:
                    # Record failed
                    temp_import.update_record_status(
                        next_index, "failed", error=error)
                    return {
                        'success': True,
                        'status': 'in_progress',
                        'progress': temp_import.progress,
                        'current_record': next_index,
                        'record_status': 'failed',
                        'error': error
                    }
                else:
                    # Record succeeded
                    temp_import.update_record_status(next_index, "success")
                    return {
                        'success': True,
                        'status': 'in_progress',
                        'progress': temp_import.progress,
                ...
                # Record failed with exception
                error_msg = str(e)
                temp_import.update_record_status(
                    next_index, "failed", error=error_msg)
                logger.error(
                    f"Error creating sale during incremental import (record {next_index}): {error_msg}")
            ...
            logger.exception(f"Error in incremental import: {str(e)}")
            db.session.rollback()
            return {
                'success': False,
                'error': str(e)
    ...
    @staticmethod
    def get_import_progress(import_id, user_id):
        """
        ...
        try:
            ...
            return {
                ...
                'updated_at': temp_import.updated_at.isoformat()
            }
    ...
    @staticmethod
    def finish_incremental_import(import_id, user_id):
        """
        Finish an incremental import process, cleanup and return final stats
        """
        try:
            # Find the temporary import record
            temp_import = TempImport.get_by_id(import_id, user_id)
            ...
            # Get the final stats
            final_stats = {
                'total': temp_import.progress.get('total', 0),
                'succeeded': temp_import.progress.get('succeeded', 0),
                'failed': temp_import.progress.get('failed', 0),
                'status': temp_import.status
            }
            ...
            ActivityService.log_activity(
                ...
                message=f"Imported {final_stats['succeeded']} sales from file ({final_stats['failed']} failed)",
                entity_type="sale",
                metadata={
                    "imported_count": final_stats['succeeded'],
            ...
            # Optionally delete the temp import to save space
            # Or keep it for a period for reference
            # db.session.delete(temp_import)
            # db.session.commit()
