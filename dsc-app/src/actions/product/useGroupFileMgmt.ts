...
import { productGroupKeys } from "@/services/product/GroupService";
import {
  getSampleGroupImportFile,
  importGroups,
  confirmImportGroups,
  cancelImportGroups,
  ImportResponse,
...
export const useGroupFileMgmt = () => {
  ...
  const handleDownloadSample = async (): Promise<boolean> => {
    ...
    try {
      ...
      const result = await getSampleGroupImportFile();
  ...
  /**
   * Import groups data from file (validation phase)
   */
  const handleImportGroups = async (
    ...
    try {
      showInfo("Validating group data, please wait...");
  ...
  const handleConfirmImport = async (): Promise<boolean> => {
    ...
    try {
      ...
      const result = await confirmImportGroups(importId);

      if (result.success) {
        showSuccess(result.msg || "Group data imported successfully");
  ...
  const handleCancelImport = async (): Promise<boolean> => {
    ...
    try {
      ...
      await cancelImportGroups(importId);
  ...
  return {
    ...
    importGroups: handleImportGroups,
